...
namespace eka2l1::drivers::camera {
    // Pixel-side contract every backend that can hand over a BGRA frame shares,
    // so a frame reaches the guest through the same packing rules whatever
    // produced it.

    // Formats such a backend can produce. Mirrors the set the Android backend
    // advertises.
