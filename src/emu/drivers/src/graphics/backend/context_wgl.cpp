...
namespace eka2l1::drivers::graphics {
    ...
    static void clear_wgl_extension_pointers() {
    ...
    gl_context_wgl::gl_context_wgl(const drivers::window_system_info& wsi, bool stereo, bool core) {
        if (!wsi.render_surface) {
            LOG_ERROR(DRIVER_GRAPHICS, "Render surface handle is null!");
            return;
        }
        ...
        if (!GetClientRect(m_window_handle, &window_rect)) {
            LOG_ERROR(DRIVER_GRAPHICS, "Fail to get client rect!");
            return;
        }

        // Clear extension function pointers before creating the first context.
        clear_wgl_extension_pointers();
        ...
        if (!m_dc) {
            LOG_ERROR(DRIVER_GRAPHICS, "(1) Can't create an OpenGL Device context. Fail.");
            return;
        }

        const int pixel_format = ChoosePixelFormat(m_dc, &pfd);
        if (pixel_format == 0) {
            LOG_ERROR(DRIVER_GRAPHICS, "(2) Can't find a suitable PixelFormat.");
            return;
        }

        if (!SetPixelFormat(m_dc, pixel_format, &pfd)) {
            LOG_ERROR(DRIVER_GRAPHICS, "(3) Can't set the PixelFormat.");
            return;
        }

        m_rc = wglCreateContext(m_dc);
        if (!m_rc) {
            LOG_ERROR(DRIVER_GRAPHICS, "(4) Can't create an OpenGL rendering context.");
            return;
        }
        ...
        if (core)
        {
            ...
            if (!wglMakeCurrent(m_dc, m_rc)) {
                LOG_ERROR(DRIVER_GRAPHICS, "(5) Can't make dummy WGL context current.");
                return;
            }
            ...
            HGLRC core_context = create_core_context(m_dc, nullptr);
            ...
            if (!wglMakeCurrent(m_dc, nullptr)) {
                LOG_ERROR(DRIVER_GRAPHICS, "(6) Failed to switch out temporary context");
                return;
            }
            ...
            if (core_context) {
                ...
                LOG_WARN(DRIVER_GRAPHICS, "Failed to create a core OpenGL context. Using fallback context.");
            }
        }
    ...
    gl_context_wgl::~gl_context_wgl() {
        if (m_rc) {
            if (wglGetCurrentContext() == m_rc && !wglMakeCurrent(m_dc, nullptr))
                LOG_ERROR(DRIVER_GRAPHICS, "Could not release drawing context.");

            
            if (!wglDeleteContext(m_rc))
                LOG_ERROR(DRIVER_GRAPHICS, "Attempt to release rendering context failed.");
        ...
        if (m_dc) {
            if (m_pbuffer_handle) {
                ...
                if (!ReleaseDC(m_window_handle, m_dc))
                    LOG_ERROR(DRIVER_GRAPHICS, "Attempt to release device context failed.");
    ...
    bool gl_context_wgl::is_headless() const {
        return !m_window_handle;
    }

    void gl_context_wgl::set_swap_interval(int interval) {
    ...
    void gl_context_wgl::swap_buffers() {
        SwapBuffers(m_dc);
    }

    std::unique_ptr<gl_context> gl_context_wgl::create_shared_context() {
        // WGL does not support surfaceless contexts, so we use a 1x1 pbuffer instead.
        HANDLE pbuffer;
        HDC dc;

        if (!create_pbuffer(m_dc, 1, 1, &pbuffer, &dc))
            return nullptr;

        HGLRC rc = create_core_context(dc, m_rc);
        if (!rc) {
        ...
        std::unique_ptr<gl_context_wgl> context = std::make_unique<gl_context_wgl>();
    ...
    HGLRC gl_context_wgl::create_core_context(HDC dc, HGLRC share_context)
    {
        if (!wglCreateContextAttribsARB) {
            LOG_WARN(DRIVER_GRAPHICS, "Missing WGL_ARB_create_context extension");
            return nullptr;
        }
        ...
        for (const auto& version : s_desktop_opengl_versions) {
            ...
            if (core_context) {
                LOG_INFO(DRIVER_GRAPHICS, "WGL: Created a GL {}.{} core context", version.first, version.second);
                return core_context;
            }
        }

        LOG_ERROR(DRIVER_GRAPHICS, "Unable to create a core OpenGL context of an acceptable version");
        return nullptr;
    }

    bool gl_context_wgl::create_pbuffer(HDC onscreen_dc, int width, int height, HANDLE* pbuffer_handle, HDC* pbuffer_dc) {
        if (!wglChoosePixelFormatARB || !wglCreatePbufferARB || !wglGetPbufferDCARB || !wglReleasePbufferDCARB || !wglDestroyPbufferARB) {
            LOG_ERROR(DRIVER_GRAPHICS, "Missing WGL_ARB_pbuffer extension");
            return false;
        }
        ...
        if (!wglChoosePixelFormatARB(onscreen_dc, pf_iattribs.data(), pf_fattribs.data(), 1, &pixel_format, &num_pixel_formats)) {
            LOG_ERROR(DRIVER_GRAPHICS, "Failed to obtain a compatible pbuffer pixel format");
            return false;
        }

        static constexpr std::array<int, 1 * 2> pb_attribs = {};

        HPBUFFERARB pbuffer = wglCreatePbufferARB(onscreen_dc, pixel_format, width, height, pb_attribs.data());
        if (!pbuffer) {
            LOG_ERROR(DRIVER_GRAPHICS,"Failed to create pbuffer");
            return false;
        }

        HDC dc = wglGetPbufferDCARB(pbuffer);
        if (!dc) {
            LOG_ERROR(DRIVER_GRAPHICS,"Failed to get drawing context from pbuffer");
    ...
    bool gl_context_wgl::make_current() {
        return wglMakeCurrent(m_dc, m_rc) == TRUE;
    }

    bool gl_context_wgl::clear_current() {
