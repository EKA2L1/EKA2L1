// File copy from PSP2 branch of RPCS3 with some modifications
// License under GPLv2
...
inline bool arm_op_thumb_is_32(uint32_t op16)
{
	return (op16 >> 11) >= 29;
}
...
namespace arm_code
{
	...
	// Bitfield aliases (because only uint32_t base type is used)
	template<uint I, uint N> using bf = bf_t<uint32_t, I, N>;
	template<uint I, uint N> using sf = bf_t<int32_t, I, N>;
	template<uint32_t V, uint N = 32> using ff = ff_t<uint32_t, V, N>;
	...
	struct nullary
	{
		static inline uint32_t extract()
		{
			return F::extract(0);
		}
	};
	...
	struct from_first
	{
		static inline uint32_t extract(uint32_t first, uint32_t)
		{
	...
	struct from_second
	{
		static inline uint32_t extract(uint32_t, uint32_t second)
		{
	...
	struct negatef : bf_base<uint32_t, N>
	{
		static inline uint32_t extract(uint32_t value)
		{
	...
	struct notf : bf_base<uint32_t, N>
	{
		static inline uint32_t extract(uint32_t value)
		{
	...
	struct xorf : bf_base<uint32_t, N>
	{
		static inline uint32_t extract(uint32_t value)
		{
	...
	// AND binary op
	template<typename F1, typename F2, uint N = F1::bitsize>
	struct andf : bf_base<uint32_t, N>
	{
		static inline uint32_t extract(uint32_t value)
		{
			return F1::extract(value) & F2::extract(value);
	...
	// SHL binary op
	template<typename F1, typename F2, uint N = F1::bitsize>
	struct shlf : bf_base<uint32_t, N>
	{
		static inline uint32_t extract(uint32_t value)
		{
			return F1::extract(value) << F2::extract(value);
	...
	struct eqcf : bf_base<uint32_t, 32>
	{
		static inline uint32_t extract(uint32_t value)
		{
	...
	struct thumb_expand_imm
	{
		...
		static uint32_t extract(uint32_t value)
		{
			const uint32_t imm12 = imm12i38::extract(value);

			if ((imm12 & 0xc00) >> 10)
			{
				const uint32_t x = (imm12 & 0x7f) | 0x80;
				const uint32_t s = (imm12 & 0xf80) >> 7;
				const uint32_t r = x >> s | x << (32 - s); // Rotate
				return r;
			}
			else
			{
	...
	{
		static uint32_t extract(uint32_t value, uint32_t carry_in)
		{
			const uint32_t imm12 = imm12i38::extract(value);
	...
	struct arm_expand_imm
	{
		...
		static uint32_t extract(uint32_t value)
		{
			const uint32_t imm12 = bf<0, 12>::extract(value);

			const uint32_t x = imm12 & 0xff;
			const uint32_t s = (imm12 & 0xf00) >> 7;
			const uint32_t r = x >> s | x << (32 - s); // Rotate
	...
	{
		static uint32_t extract(uint32_t value, uint32_t cond)
		{
			return cond != 0xf;
		}
	};

	enum SRType : uint32_t
	{
		SRType_LSL,
		SRType_LSR,
		SRType_ASR,
	...
	struct decode_imm_shift_type
	{
		static uint32_t extract(uint32_t value)
		{
			const uint32_t type = T::extract(value);
			const uint32_t imm5 = N::extract(value);
...
// TODO
#define SKIP_IF(cond) [](uint32_t c) -> bool { return cond; }
...
class arm_decoder
{
	...
	struct instruction_info
	{
		...
		uint32_t code;
		T pointer;
		bool(*skip)(uint32_t code); // TODO (if not nullptr, must be specified yet)

		bool match(uint32_t op) const
		{
...
public:
	arm_decoder()
	{
		...
		for (uint32_t i = 0xe800; i < 0x10000; i++)
		{
			if (m_op16_table[i]) LOG_ERROR("Invalid m_op16_table entry 0x%04x", i);
		...
		{
			LOG_TRACE("Set found (%u):", s.size());
			for (const auto& e : s)
			{
				LOG_TRACE("** 0x%08x, 0x%08x", e->mask, e->code);
			}
		}
	}
	...
	// Second step
	T decode_thumb(uint32_t op32) const
	{
	...
	T decode_arm(uint32_t op) const
	{
