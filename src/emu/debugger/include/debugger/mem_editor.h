...
struct MemoryEditor {
    ...
    bool OptShowHexII; // = false  // display values in HexII representation instead of regular hexadecimal: hide nullptr/zero bytes, ascii values as ".X".
    bool OptShowAscii; // = true   // display ASCII representation on the right side.
    bool OptGreyOutZeroes; // = true   // display nullptr/zero bytes using the TextDisabled color.
    ...
    eka2l1::process_ptr CurrentProcess;
    int DefaultProcessNum;
    int DefaultMemoryRegion;

    MemoryEditor(eka2l1::kernel_system *SystemKernel) {
        Kern = SystemKernel;
    ...
    void DrawWindow(const char *title) {
        Sizes s;
        CalcSizes(s, 0xFFFFFFFF, 0x00000000);
        ImGui::SetNextWindowSizeConstraints(ImVec2(0.0f, 0.0f), ImVec2(s.WindowWidth, FLT_MAX));

        Open = true;
        if (ImGui::Begin(title, &Open, ImGuiWindowFlags_NoScrollbar)) {
            const auto processes = Kern->get_all_processes();
            std::vector<std::string> processes_name_list;

            for (const auto &process : processes) {
                processes_name_list.push_back(process->name());
            }

            if (!CurrentProcess) {
                CurrentProcess = processes[DefaultProcessNum];
            }

            if (ImGui::BeginCombo("Processes", processes_name_list[DefaultProcessNum].c_str())) {
                for (std::size_t i = 0; i < processes_name_list.size(); i++) {
                    if (ImGui::Selectable(processes_name_list[i].c_str(),
                            CurrentProcess ? CurrentProcess->name() == processes_name_list[i] : false)) {
            ...
            if (ImGui::BeginCombo("Memory region", "Memory region")) {
                const char *option[] = { "Local data", "Shared data", "RAM code", "ROM" };

                for (size_t i = 0; i < 4; i++) {
                    if (ImGui::Selectable(option[i], i == DefaultMemoryRegion)) {
                        DefaultMemoryRegion = i;
                    }
                }
            ...
            switch (DefaultMemoryRegion) {
            case 0:
                DrawContents(local_data, dll_static_data - local_data);
                break;
    ...
    void DrawContents(const std::size_t base, const std::size_t mem_size) {
        if (!CurrentProcess) {
            return;
        }

        Sizes s;
        CalcSizes(s, mem_size, base);
        ImGuiStyle &style = ImGui::GetStyle();
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
        {
            ...
            ImGui::Text(format_address, s.AddrDigitsCount, base + addr);
        ...
        if (OptShowOptions) {
            ...
            if (GotoAddr != (size_t)-1) {
                if (GotoAddr < mem_size) {
                    ...
                    float m = ImGui::GetCursorStartPos().y + (GotoAddr / Cols) * ImGui::GetTextLineHeight();
                    ImGui::SetScrollFromPosY(m);
                    ImGui::EndChild();
