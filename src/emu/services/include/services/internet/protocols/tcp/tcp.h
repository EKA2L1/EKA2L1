...
namespace eka2l1::epoc::internet {
    ...
    class tcp_host_resolver : public socket::host_resolver {
    private:
        tcp_protocol *papa_;

    public:
        explicit tcp_host_resolver(tcp_protocol *papa);
    ...
    class tcp_protocol : public socket::protocol {
    public:
        explicit tcp_protocol(const bool oldarch)
        ...
        virtual std::unique_ptr<epoc::socket::host_resolver> make_host_resolver() override {
            return std::make_unique<tcp_host_resolver>(this);
        }
    };
}
