...
int main(int argc, char **argv) {
    ...
    if (argc <= 1) {
        ...
        LOG_INFO(SYSTEM, "Usage: mbm2bmp [filename].");
    ...
    if (!mbm_ptr) {
        LOG_ERROR(SYSTEM, "Try to map MBM file to memory but failed, exiting...");
        return -2;
    }
    ...
    if (!mbmf.do_read_headers()) {
        LOG_ERROR(SYSTEM, "Reading MBM header failed! At least one magic value doesn't match!");
        return -3;
    }
    ...
    for (std::size_t i = 0; i < mbmf.trailer.count; i++) {
        ...
        if (!mbmf.save_bitmap_to_file(i, bmp_name.c_str())) {
            LOG_ERROR(SYSTEM, "Can't save bitmap number {}!", i);
        }
    }
